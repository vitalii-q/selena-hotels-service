...
echo "⏳ Waiting for CockroachDB at ${HOTELS_COCKROACH_HOST}:${HOTELS_COCKROACH_PORT}..."
until nc -z "$HOTELS_COCKROACH_HOST" "$HOTELS_COCKROACH_PORT"; do
...
cockroach sql --insecure --host="$HOTELS_COCKROACH_HOST" --port="$HOTELS_COCKROACH_PORT" --execute="SELECT 1;"
...
if ! cockroach sql --insecure --host="$HOTELS_COCKROACH_HOST" --port="$HOTELS_COCKROACH_PORT" \
  ...
  cockroach sql --insecure --host="$HOTELS_COCKROACH_HOST" --port="$HOTELS_COCKROACH_PORT" --execute="
